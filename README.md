...
Интеграционный тест использует PostgreSQL, временно создаёт и удаляет тестовый заказ №12293 и тестовых сборщиков. Он проверяет журнал успешного, повторного и ошибочного импорта, dashboard, аналитику, распределение `16/16/16`, завершение, проблемную позицию, запрет закрытия, решение проблемы, закрытие, историю, аудит и две PDF-формы:
