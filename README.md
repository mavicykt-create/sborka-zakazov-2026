...
Монорепозиторий для складского сервиса сборки заказов. Текущая версия принимает товарный чек XLSX, распределяет его между сборщиками, сопровождает проверку и закрытие заказа и формирует итоговые PDF.
...
Интеграционный тест использует PostgreSQL, временно создаёт и удаляет тестовый заказ №12293 и тестовых сборщиков. Он проверяет распределение `16/16/16`, завершение, проблемную позицию, запрет закрытия, решение проблемы, закрытие, историю, аудит и две PDF-формы:
