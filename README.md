...
19. Голосовая сборка с русскими командами, TTS и восемью локальными звуковыми сигналами.
...
Точные русские команды: «Взял», «Повтори», «Сколько», «Не нашёл», «Пропусти», «Отмени», «Осталось», «Пауза» и «Продолжить». Команда «Дальше» намеренно не подтверждает товар. После «Пауза» продолжить работу можно большой экранной кнопкой; все результативные действия доступны кнопками и без Web Speech API.

Сигнал «Принято» звучит только после успешного ответа backend. При сетевой ошибке текущая позиция не меняется, переход не выполняется, а интерфейс показывает просьбу повторить команду. Позиции `REVIEW` заблокированы до решения администратора. В панели «Проверить сигналы» можно отдельно прослушать все восемь локальных сигналов.
