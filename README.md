...
из самого подключённого ящика, задайте `IMAP_MAILBOX=SENT` (реальная папка определится автоматически) и
`IMAP_UNSEEN_ONLY=false`, потому что отправленные письма уже имеют признак прочитанных. При первом запуске
сервис берёт только письма за последние 15 минут; окно задаётся через `IMAP_LOOKBACK_MINUTES`.
