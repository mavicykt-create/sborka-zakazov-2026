...
app.post('/api/orders/import-xlsx', async (request, reply) => {
  ...
  if (!file.filename.toLowerCase().endsWith('.xlsx')) return reply.code(400).send({ error: 'Нужен файл .xlsx' });
  ...
  const parsed = await parseOrderXlsx(buffer);
...
app.get('/api/orders', async () => db.order.findMany({
